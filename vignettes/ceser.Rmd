---
title: "Introcution to ceser"
output: rmarkdown::html_vignette
bibliography: /home/diogo/Dropbox/CienciasSociais/references/references.bib
vignette: >
...
\end{split}
\end{align}  where \(\iota_{g}\) is a unitary column vector, \(I_g\) is a \(g \times g\) identity matrix, and \(P_g = X_g (X^{T}X)^{-1}X_{g}^{T}\). Equation (\ref{eq-exp-ee}) can be rewriten concisely as:
...
We can rewrite the equation (\ref{eq-cese-concise}) as:
...
To ilustrate how to use the **ceser** package, we use the data set `dcese`, which is provided with the package. The data set contains information of 310 (i=1,\ldots{}, 310) observations across 51 countries (g=1,\ldots{},51).
...
For all the examples below, we use the HC3 correction. To compute the variance covariance matrix of the estimated coefficients \(\hat{\beta}\), just execute:
